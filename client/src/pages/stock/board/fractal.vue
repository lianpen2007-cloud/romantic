    ...
    <div class="c-fractal">
        <div class="charts">
            <div class="chart_wrapper">
                <div id="fractal_price" class="chart" />
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    async mounted() {
        this.chart = echarts.init(document.getElementById('fractal_price'))
    ...
    methods: {
        paint() {
            ...
            document.getElementById('fractal_price').style.height = height + 'px'
            const width = Math.max(this.mergedKLineList.length * 10, 800)
            document.getElementById('fractal_price').style.width = Math.max(document.body.clientWidth, width) + 'px'
            this.drawchart()
        },
        drawchart() {
...
<style>
.c-fractal .charts .chart_wrapper {
    width: 100%;
    overflow-x: scroll;
}
.c-fractal .charts .chart {
    width: 100%;
    height: 440px;
    margin-top: -40px;
}
.c-fractal .charts .volumn_chart {
    width: 100%;
    height: 220px;
    margin-top: -100px;
    position: relative;
